...
namespace MVCProject.Controllers
{
    public class CompanyJobDescriptionController : Controller
    {
        ...
        public ActionResult Index(Guid? Id)
        {
            if (Id == null)
            {
                var companyJobDescriptions = db.CompanyJobDescription.Include(c => c.CompanyJob);
                return View(companyJobDescriptions.ToList());
            }
            else
            {
                var companyJobDescription = db.CompanyJobDescription.Where(c => c.Job == Id).Include(c => c.CompanyJob);
        ...
        public ActionResult Create([Bind(Include = "Id,Job,JobName,JobDescriptions")] CompanyJobDescriptionPoco companyJobDescriptionPoco)
        {
            if (ModelState.IsValid)
            {
                ...
                db.SaveChanges();
                return RedirectToAction("Index", new {id = companyJobDescriptionPoco.Job });
            }
        ...
        public ActionResult Edit([Bind(Include = "Id,Job,JobName,JobDescriptions")] CompanyJobDescriptionPoco companyJobDescriptionPoco)
        {
            if (ModelState.IsValid)
            {
                ...
                db.SaveChanges();
                return RedirectToAction("Index", new { id = companyJobDescriptionPoco.CompanyJob });
            }
            ViewBag.Job = new SelectList(db.CompanyJobs, "Id", "Id", companyJobDescriptionPoco.Job);
            return View(companyJobDescriptionPoco);
        }
